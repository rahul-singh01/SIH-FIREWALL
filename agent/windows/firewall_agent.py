...
class WindowsFirewallAgent:
    ...
    def initialize(self):
        logger.info("Initializing Windows Firewall Agent")
        try:
            self.wfp.initialize()
    ...
    def set_default_chain_policy(self):
        ...
        try:
            current_policy = self.wfp.get_chain_policy()
            if current_policy != default_policy:
                self.wfp.set_chain_policy(default_policy)
    ...
    def signal_handler(self, signum, frame):
        logger.info(f"Received signal {signum}. Stopping Windows Firewall Agent")
        self.running = False
    ...
    def apply_app_specific_rules(self, process, app):
        try:
            ...
            self.wfp.add_app_specific_rule(app.name, process['pid'], app.allowed_ports)
    ...
    def apply_policies(self, new_policies):
        try:
            with self.lock:
                current_rules = self.wfp.list_rules()
    ...
    def remove_obsolete_rules(self, new_policies, current_rules):
        ...
        for rule in current_rules.split('\n'):
            if rule and rule not in new_rule_set:
                try:
                    handle = self.wfp.get_rule_handle(rule.split())
                    if handle:
                        self.wfp.delete_rule(handle)
    ...
    def collect_and_send_logs(self):
        try:
            ...
            logger.error(f"Error collecting or sending logs: {str(e)}")
            self.alert("medium", f"Log collection error: {str(e)}")
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.error(f"Error cleaning up Windows Firewall: {str(e)}")
        finally:
            self.db.close()
